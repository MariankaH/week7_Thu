...
data_march2024 %>% 
  ...
  summarize(sum_quantity = sum(paid_quantity, na.rm = TRUE)) %>% 
...
joineddata <- full_join(data_march2024, healthboard_names_data, by = c("hbt" = "hb"))
...
```{r}
missing_data <- joineddata %>% 
  filter(is.na(hb_name))

# all of them are from the Scottish Ambulance Service = no specific area
```
...
```{r}
healthboard_population <- full_join(joineddata, population_data, by = "hb_name")
healthboard_population %>% 
  select(hb_name, hb_population)
...
```{r}
paracetamol_data <- healthboard_population %>% 
...
```{r}
paracetamol_capsules <- healthboard_population %>% 
  filter(str_detect(bnf_item_description, "PARACETAMOL 500MG CAP")) %>% 
  group_by(hb_name) %>% 
  summarise(paid_quantity = sum(paid_quantity), hb_population = first(hb_population)) %>% 
  mutate(ratio = paid_quantity / hb_population) %>% 
  ggplot(aes(x = reorder(hb_name, ratio), y = ratio, fill = hb_name)) +
  geom_col() +
  coord_flip() +
  labs(x = "Healthboard Name", y = "Number of Capsules/Caplets per Person")

paracetamol_capsules
```
...
may2020_names <- full_join(may2020, healthboard_names_data, by = c("HBT" = "hb"))
may2020_population <- full_join(may2020_names, population_data, by = "hb_name")

may2020_paracetamol <- may2020_population %>% 
  filter(str_detect(BNFItemDescription, "PARACET_TAB|PARACET_CAP")) %>% 
  group_by(hb_name) %>% 
  summarise(PaidQuantity = sum(PaidQuantity), hb_population = first(hb_population)) %>% 
  mutate(ratio = PaidQuantity / hb_population) %>% 
  ggplot(aes(x = reorder(hb_name, ratio), y = ratio, fill = hb_name)) +
  geom_col() +
  coord_flip() +
  labs(x = "Healthboard Name", y = "Number of Capsules/Caplets per Person")
...
may2020_paracetamol + paracetamol_capsules
```
